...
function script_properties()
    ...
    obs.obs_properties_add_button(props, 'button', 'Go', trigger)
    return props
end
...
function script_update(settings)
    ...
    zoomingActive = obs.obs_data_get_bool(settings, 'active')
...
function script_tick(seconds)
    counter = counter + 1
    -- if counter % 75 == 0 then
        -- print('writing screenshot')
        -- local src = obs.obs_get_source_by_name('Closeup')
        -- obs.obs_frontend_take_source_screenshot(src)
        -- obs.obs_source_release(src)
        -- scandir('F:/python/kates')

    -- end

    if counter % 10 == 0 and not active then
        file = io.open ("C:/paintingmusic/scripts/position.txt")
        io.input(file)
        -- read the current coordinates file
        local x = io.read()
        local y = io.read()
        local w = io.read()
        local h = io.read()

        if zoomingActive == false then
            
            x = 960.0
            y = 540.0
            w = 1929.6
            h = 1085.3999999999999
        ...
        if x ~= nil then
            if x ~= curX or y ~= curY or w ~= curW or h ~= curH then
